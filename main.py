...
def main():
    ...
    pg.display.set_caption('Ультра Мега Тетрис')
    showText('Ультра Мега Тетрис')
    while True:
        runTetris()
        pauseScreen()
        showText('Игре пришёл конец!!!')
...
def showText(text):
    ...
    pressKeySurf, pressKeyRect = txtObjects('Не скучно? Ну тогда нажми любую клавишу', basic_font, title_color)
...
def drawTitle():
    titleSurf = big_font.render('Ультра Мега Тетрис', True, title_color)
    titleRect = titleSurf.get_rect()
    titleRect.topleft = (window_w - 525, 20)
    display_surf.blit(titleSurf, titleRect)
